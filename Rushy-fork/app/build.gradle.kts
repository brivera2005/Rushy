...
android {
    ...
    buildTypes {
        debug {
            // Keep com.rushy.app package id so OTA and launcher label stay "Rushy".
...
val defaultCredentialsFile = file("src/main/java/com/streamvault/app/DefaultCredentials.kt")
val defaultCredentialsExample = file("src/main/java/com/streamvault/app/DefaultCredentials.kt.example")

tasks.register("ensureDefaultCredentials") {
    doLast {
        if (!defaultCredentialsFile.exists()) {
            if (!defaultCredentialsExample.exists()) {
                throw GradleException("Missing DefaultCredentials.kt.example for local build.")
            }
