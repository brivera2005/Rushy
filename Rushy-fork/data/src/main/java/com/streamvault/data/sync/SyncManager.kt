...
import com.streamvault.data.remote.plex.PlexProvider
...
class SyncManager @Inject constructor(
    ...
    private val plexProvider: PlexProvider,
            ...
            try {
                val outcome = withContext(Dispatchers.IO) {
                    when (provider.type) {
                        ...
                        ProviderType.PLEX -> syncPlex(provider, force, onProgress)
                    }
                }
                ...
                if (provider.type == ProviderType.XTREAM_CODES) {
                    maybeSyncPlexFallback(onProgress)
                }
                com.streamvault.domain.model.Result.error(safeMessage, e)
            }
        } finally {
    ...
    private suspend fun syncPlex(
        ...
        val warnings = mutableListOf<String>()
        try {
            val decryptedToken = credentialCrypto.decryptIfNeeded(provider.password)
            val decryptedProvider = provider.copy(password = decryptedToken)
            progress(provider.id, onProgress, "Loading Plex library...")
            val (resolvedMovies, resolvedSeries) = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
                plexProvider.fetchMovies(decryptedProvider) to plexProvider.fetchSeries(decryptedProvider)
            }
            if (resolvedMovies is com.streamvault.domain.model.Result.Error && resolvedSeries is com.streamvault.domain.model.Result.Error) {
                warnings.add("Failed to load Plex catalog: ${resolvedMovies.message}; ${resolvedSeries.message}")
                return SyncOutcome(partial = true, warnings = warnings)
            }
            val movieEntities = (resolvedMovies as? com.streamvault.domain.model.Result.Success)?.data.orEmpty()
            val seriesEntities = (resolvedSeries as? com.streamvault.domain.model.Result.Success)?.data.orEmpty()
            if (movieEntities.isNotEmpty()) {
                progress(provider.id, onProgress, "Importing Plex movies...")
                movieEntities.chunked(50).forEach { chunk ->
                    transactionRunner.inTransaction {
                        movieDao.upsertCategoryPage(provider.id, chunk)
                    }
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, com.streamvault.domain.model.ContentType.MOVIE.name, listOf(
                        com.streamvault.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 1L, name = "Plex Movies",
                            type = com.streamvault.domain.model.ContentType.MOVIE
                        )
                    ))
                }
            ...
            if (seriesEntities.isNotEmpty()) {
                progress(provider.id, onProgress, "Importing Plex series...")
                seriesEntities.forEach { seriesEntity ->
                    val remoteId = seriesEntity.providerSeriesId
                    if (!remoteId.isNullOrBlank()) {
                        transactionRunner.inTransaction {
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, com.streamvault.domain.model.ContentType.SERIES.name, listOf(
                        com.streamvault.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 2L, name = "Plex Shows",
                            type = com.streamvault.domain.model.ContentType.SERIES
                        )
                    ))
                }
            ...
            if (movieEntities.isEmpty() && seriesEntities.isEmpty()) {
                warnings.add("Plex library is empty or contains no supported movies or series.")
                return SyncOutcome(partial = true, warnings = warnings)
            }
            return SyncOutcome(warnings = warnings)
        } catch (e: Exception) {
            warnings.add("Plex sync failed: ${e.message.orEmpty()}")
            return SyncOutcome(partial = true, warnings = warnings)
        }
    }

    private suspend fun maybeSyncPlexFallback(onProgress: ((String) -> Unit)?) {
        val plexProviders = providerDao.getByTypeSync(ProviderType.PLEX)
        val plexProvider = plexProviders.firstOrNull() ?: return
        try {
            Log.i(TAG, "Xtream sync failed — triggering Plex fallback sync for provider ${plexProvider.id}")
            syncPlex(plexProvider.toDomain().copy(password = credentialCrypto.decryptIfNeeded(plexProvider.password)), force = true, onProgress = onProgress)
            providerDao.updateSyncTime(plexProvider.id, System.currentTimeMillis())
        } catch (e: Exception) {
            Log.w(TAG, "Plex fallback sync failed: ${e.message}")
        }
    }
    ...
    private suspend fun syncLiveOnly(
        ...
        when (provider.type) {
            ...
            ProviderType.JELLYFIN,
            ProviderType.PLEX -> throw IllegalStateException("Live TV retry is unavailable for ${provider.type.name} providers")
        }
    ...
    private suspend fun syncMoviesOnly(
        ...
        when (provider.type) {
            ...
            ProviderType.JELLYFIN,
            ProviderType.PLEX -> throw IllegalStateException("Movies retry is unavailable for ${provider.type.name} providers")
        }
        return if (sectionWarnings.isEmpty()) SyncOutcome() else SyncOutcome(partial = true, warnings = sectionWarnings)
    }
    ...
    private suspend fun syncSeriesOnly(
        ...
        when (provider.type) {
            ...
            ProviderType.JELLYFIN,
            ProviderType.PLEX -> throw IllegalStateException("Series retry is unavailable for ${provider.type.name} providers")
        }
        return if (sectionWarnings.isEmpty()) SyncOutcome() else SyncOutcome(partial = true, warnings = sectionWarnings)
    }
