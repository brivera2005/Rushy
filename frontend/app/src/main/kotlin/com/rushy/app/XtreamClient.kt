...
class XtreamClient(
    ...
    suspend fun fetchLiveStreams(categoryMap: Map<String, String> = emptyMap()): List<MediaItem> {
        return runCatching {
            api.getLiveStreams(username, password).mapNotNull { dto ->
                dto.toLiveMediaItem(categoryMap)
            }
        }.getOrElse { e ->
            Log.e(TAG, "fetchLiveStreams failed", e)
    ...
    suspend fun fetchVodStreams(categoryMap: Map<String, String> = emptyMap()): List<MediaItem> {
        return runCatching {
            api.getVodStreams(username, password).mapNotNull { dto ->
                dto.toVodMediaItem(categoryMap)
            }
        }.getOrElse { e ->
            Log.e(TAG, "fetchVodStreams failed", e)
